...
import static org.dl.util.ActivationFunction.softmax;
...
public class MultiClassLogisticRegression {
	...
	public MultiClassLogisticRegression(int nIn, int nOut) {
		this.nIn = nIn;
		this.nOut = nOut;
	...
	public double[][] train(double[][] X, int Y[][], int minibatchSize, double learningRate) {
	...
	public static void main(String[] args) {
		...
		int[][] train_Y = new int[train_N][nOut];

		double[][] test_X = new double[test_N][nIn];
		Integer[][] test_Y = new Integer[test_N][nOut];
		Integer[][] predicted_Y = new Integer[test_N][nOut];
		...
		int[][][] train_Y_minibatch = new int[minibatch_N][minibatchSize][nOut]; // minibatches of output data for training
		List<Integer> minibatchIndex = new ArrayList<>(); // data index for minibatch to apply SGD
		for (int i = 0; i < train_N; i++)
			minibatchIndex.add(i);
		Collections.shuffle(minibatchIndex, random); // shuffle data index for SGD
		...
		for (int i = 0; i < minibatch_N; i++) {
			for (int j = 0; j < minibatchSize; j++) {
				...
				train_Y_minibatch[i][j] = train_Y[minibatchIndex.get(i * minibatchSize + j)];
			}
		}
		...
		MultiClassLogisticRegression classifier = new MultiClassLogisticRegression(nIn, nOut);

		// train
		for (int epoch = 0; epoch < epochs; epoch++) {
			for (int batch = 0; batch < minibatch_N; batch++) {
				classifier.train(train_X_minibatch[batch], train_Y_minibatch[batch], minibatchSize, learningRate);
			}
			learningRate *= 0.95;
		}
		...
		for (int i = 0; i < test_N; i++) {
			int predicted_ = Arrays.asList(predicted_Y[i]).indexOf(1);
			int actual_ = Arrays.asList(test_Y[i]).indexOf(1);
